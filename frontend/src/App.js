...
function App() {
  ...
  return (
    <div className={`min-h-screen transition-all duration-300 ${isDarkMode ? 'dark bg-gray-900' : 'bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100'}`}>
              ...
              {processedServices.map((service) => (
                <Card key={service.id} className={`group hover:shadow-xl transition-all duration-300 backdrop-blur-md border overflow-hidden hover-lift ${isDarkMode ? 'bg-gray-800/70 border-gray-700/20 hover:bg-gray-800/90' : 'bg-white/70 border-white/20 hover:bg-white/90'}`}>
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <CardTitle className={`text-lg sm:text-xl font-bold transition-colors group-hover:text-indigo-600 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'} truncate`}>
                            {service.name}
                            {isCheapest(service) && (
                              <Trophy className="inline-block ml-2 h-4 w-4 text-yellow-500 flex-shrink-0" />
                            )}
                          </CardTitle>
                          <Badge className={`mt-1 ${getCategoryColor(service.category)} text-xs`}>
                            {service.category}
                          </Badge>
                        </div>
                      </div>
                    </div>
                    <CardDescription className={`mt-2 leading-relaxed text-sm line-clamp-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      {service.description}
                    </CardDescription>
                  </CardHeader>
                      ...
                      <div className="space-y-1.5 sm:space-y-2">
                        {service.tiers.slice(0, 3).map((tier, index) => (
                          <div key={index} className={`flex justify-between items-center p-2 rounded-lg ${isDarkMode ? 'bg-gray-700/60' : 'bg-gray-50/80'}`}>
                            <span className={`font-medium text-xs sm:text-sm truncate ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{tier.name}</span>
                            <span className="font-bold text-indigo-600 text-xs sm:text-sm ml-2 flex-shrink-0">{formatPrice(tier.price)}</span>
                          </div>
                        ))}
                  ...
                  <CardFooter className="pt-3 sm:pt-4">
                    <Button 
                      className="w-full h-11 sm:h-10 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white group-hover:shadow-lg transition-all duration-300 text-sm sm:text-base"
                ...
                <Table className="min-w-full">
                  <TableHeader>
                    <TableRow className={isDarkMode ? 'bg-gray-800/80 border-gray-700' : 'bg-gray-50/80'}>
                      <TableHead className={`font-bold text-xs sm:text-sm whitespace-nowrap ${isDarkMode ? 'text-gray-200' : ''}`}>Service</TableHead>
                      <TableHead className={`font-bold text-xs sm:text-sm whitespace-nowrap ${isDarkMode ? 'text-gray-200' : ''}`}>Category</TableHead>
                      <TableHead className={`font-bold text-xs sm:text-sm whitespace-nowrap ${isDarkMode ? 'text-gray-200' : ''}`}>Starting Price</TableHead>
                      <TableHead className={`font-bold text-xs sm:text-sm whitespace-nowrap hidden sm:table-cell ${isDarkMode ? 'text-gray-200' : ''}`}>Top Features</TableHead>
                      <TableHead className={`font-bold text-xs sm:text-sm whitespace-nowrap ${isDarkMode ? 'text-gray-200' : ''}`}>Link</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {processedServices.map((service) => (
                      <TableRow key={service.id} className={`transition-colors ${isDarkMode ? 'hover:bg-gray-700/60 border-gray-700' : 'hover:bg-white/60'}`}>
                        <TableCell className="min-w-0">
                          <div className="flex items-center gap-2 sm:gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className={`font-semibold flex items-center gap-2 text-sm ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
                                <span className="truncate">{service.name}</span>
                                {isCheapest(service) && (
                                  <Trophy className="h-4 w-4 text-yellow-500 flex-shrink-0" />
                                )}
                              </div>
                              <div className={`text-xs sm:text-sm line-clamp-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{service.description}</div>
                            </div>
                          </div>
                        </TableCell>
                        ...
                        <TableCell className="whitespace-nowrap">
                          <div className="font-bold text-indigo-600 text-xs sm:text-sm">
                        ...
                        <TableCell>
                          <Button 
                            ...
                            variant="outline"
                            onClick={() => window.open(service.link, '_blank')}
                            className={`transition-all duration-300 h-8 w-8 sm:h-8 sm:w-auto sm:px-3 ${isDarkMode ? 'hover:bg-gray-700 hover:text-indigo-400 hover:border-indigo-400' : 'hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-300'}`}
                          >
      ...
      {/* Footer */}
      <footer className={`backdrop-blur-md border-t py-6 sm:py-8 mt-12 sm:mt-16 transition-all duration-300 ${isDarkMode ? 'bg-gray-800/70 border-gray-700/20' : 'bg-white/70 border-white/20'}`}>
